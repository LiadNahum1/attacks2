#207434044 - level1 - 536a0ffbb9f89cb807b2e3a565bc3ca2
#318841285 - level1 - 0b323015266587b7422b4735dee4b9cc [41, 45, 49, 53, 42, 46, 50, 54, 43, 47, 51, 55, 44, 48, 52, 56]
#318841285 - level2 - 7e680f4a466e17b864e561013eeac270 [41, 45, 49, 53, 42, 46, 50, 54, 43, 47, 51, 55, 44, 48, 52, 56]
import json
import numpy as np
import time
...
KEY_LENGTH = 16
FILE_NAME = "318841285/traces_318841285_4.txt"
AES_SBOX = [
...
def byte_process(key_byte, key_array):
    ...
    differences = np.zeros(shape=(256, num_of_leaks))

    traces_class = np.zeros(shape=(num_traces, 256))
    for byte_guess in range(256):
        ...
        traces_class[:, byte_guess] = v_apply_hw(sboxed_bytes)

        for i in range(num_of_leaks):
            differences[byte_guess, i] = np.corrcoef(traces_class[:, byte_guess].T, traces[:, i].T)[0][1]

    print(f'10000X256 {traces_class}')
    print(f'256X64 {differences}')
...
def main():
    ...
    print(''.join(list(map(lambda x: "{:02x}".format(x), key))))
